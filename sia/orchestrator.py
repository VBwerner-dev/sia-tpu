...
def _run_feedback_agent(
    ...
    Args:
        ...
        training_backend: "tinker-api" (default) or "tpu-native" for TPU-native mode
    """
...
def main():
    ...
    if args.focus == "weights":
        if args.training_backend == "tpu-native":
            # TPU-native mode does not need Tinker API or Modal
            logger.info("  ✓ TPU-native backend: TINKER_API_KEY and MODAL_TOKEN checks skipped.")
            logger.info("  ✓ Make sure TPU runtime (libtpu) is loaded on this machine.")
        else:
            # Tinker API mode
    ...
    # Log focus mode and training config
    ...
    if args.focus == "weights":
        ...
        logger.info(f"  - Training backend: {args.training_backend}")
