...
def _add_run_args(parser: argparse.ArgumentParser, env_config: Config) -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="tinker-api",
        choices=["tinker-api", "tpu-native"],
        help="Training backend for weight updates: 'tinker-api' (default, uses tinker-cookbook cloud API) or 'tpu-native' (runs GRPO natively on TPU via torch-xla).",
    )
    parser.add_argument(
        "--log-level",
        dest="log_level",
